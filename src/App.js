...
import ProgressScreen from './components/lessons/ProgressScreen';
...
const App = () => {
  ...
  const getLanguageData = () => {
    if (selectedLanguage === 'spanish') return spanishStages;
    if (selectedLanguage === 'creek') return creekStages;
    return spanishStages;
  };

  // No language selected - show language hub
  if (!selectedLanguage) {
    return <LanguageHub onSelectLanguage={handleLanguageSelect} />;
  }
  ...
  if (!selectedLesson) {
    return (
      <Hub 
        ...
        onSelectProgress={() => setShowProgress(true)}
      />
    );
  }
  ...
  if (selectedLesson === 4 && selectedPractice?.startsWith('infinitives')) {
    const practiceType = selectedPractice.split('-')[1];
    return (
      <InfinitivesLesson
        onBack={() => setSelectedPractice(null)}
        languageData={getLanguageData()}
  ...
  // ========================================
  // GRAMMAR LESSON (Lesson 5) ROUTING
  // ========================================
