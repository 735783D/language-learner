...
const PresentTenseLesson = ({ onBack, languageData }) => {
  ...
  const exercises = useMemo(() => {
    if (!stageData?.subLessons?.present?.exercises) return [];
    
    const allExercises = stageData.subLessons.present.exercises;
    
    // Create exercises: show infinitive, pick present tense form
    return [...allExercises]
      .sort(() => Math.random() - 0.5)
      .slice(0, 15)
      .map(verb => {
        // Get 3 wrong present tense forms
        const wrongAnswers = allExercises
          .filter(v => v.present !== verb.present)
          .sort(() => Math.random() - 0.5)
          .slice(0, 3)
          .map(v => v.present);
        
        // Shuffle all options
        const options = [verb.present, ...wrongAnswers]
          .sort(() => Math.random() - 0.5);
  ...
  return (
    <div className={`fixed inset-0 ${theme.bg} overflow-hidden`} style={{userSelect: 'none', WebkitUserSelect: 'none'}}>
      ...
      <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-40 text-center">
        <h1 className={`text-2xl font-bold ${theme.text}`}>Present Tense</h1>
        <p className={`text-sm ${theme.textSecondary}`}>I am doing...</p>
      </div>
      ...
      <div className="absolute inset-0 pt-32 pb-8 flex flex-col items-center justify-center">
        <div className="space-y-8 max-w-2xl w-full px-8">
          ...
          <div className="text-center space-y-4">
            ...
            <div className={`${theme.card} p-8 rounded-3xl shadow-lg space-y-2`}>
              ...
              <div className={`text-sm ${theme.accent} font-semibold mt-4`}>
                → "I am {exercise.translation.replace('to ', '')}..."
              </div>
            </div>
          </div>
