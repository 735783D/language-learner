...
const MatchExercise = ({ 
  ...
  // Determine the correct answer(s) for highlighting
  const correctAnswers = exercise.translation.toLowerCase().split(/[/,]/).map(t => t.trim());
  
  const isCorrectAnswer = (option) => {
    return correctAnswers.some(answer => answer === option.toLowerCase());
  };
  ...
  return (
    <div className="relative h-full w-full pt-8">
      ...
      {shuffledOptions.map((option, idx) => {
        ...
        // Show green highlight if feedback is incorrect and this is the correct answer
        const showCorrectHighlight = feedback === "incorrect" && isCorrect;
          ...
          <div
            ...
            className={`absolute ${positions[idx]} w-36 h-28 p-3 rounded-xl text-3xl font-bold flex items-center justify-center text-center transition-all border-4 cursor-move z-0 ${
              matchDroppedOn === option
                ...
                ? "bg-green-500 text-white border-green-600 scale-110 animate-pulse"
