...
const CompletionModal = ({ 
  ...
  onBack,
  lessonName = "Lesson",
  lessonId = null   // <-- NEW: pass this from the parent lesson component
}) => {
  const { theme } = useTheme();
  const { saveProgress } = useProgress();

  // Save progress whenever the modal opens with a valid lessonId
  useEffect(() => {
    if (isOpen && lessonId) {
      saveProgress(lessonId, score, totalQuestions);
    }
  }, [isOpen, lessonId, score, totalQuestions, saveProgress]);
  ...
  const getColorClasses = () => {
    if (percentage >= 80) return { border: 'border-green-500', text: 'text-green-500' };
    if (percentage >= 60) return { border: 'border-yellow-500', text: 'text-yellow-500' };
